...
4. Keep in mind that, by default, all responsive utilities are set to apply at `min-width`. That is, the `sm:` prefix does not apply only to small viewports; it applies to small breakpoints _and larger_. So you can use `md:flex` instead of `md:flex lg:flex xl:flex` for example. They both have the same behavior.
