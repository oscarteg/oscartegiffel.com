...
function Button(props: Props): JSX.Element {
  if (props.href) {
    return <a href={props.href} onClick={props.onClick}>Click me</a>;
  }
...
It ensures that only valid combinations of props are used. You can't pass both onClick and href props simultaneously.
It provides clear guidance to the user of the component on which props are allowed.
It improves type safety, as TypeScript will catch any incorrect usages at compile-time.
...
In summary, discriminated unions are a handy pattern for enhancing React props and reducers. They provide clearer guidelines, prevent common errors, and help developers build more robust and maintainable React components and reducers.
