...
description: My personal code taste — the philosophies and preferences that guide how I build software.
subtitle: My personal code taste
edition: "2026-08-06"
markdownUrl: /taste.md
...
This is my code taste, written down. Not rules, a compass. Five chapters, thirty-four verses, each one a position I arrived at by building software and getting it wrong first. Some are borrowed from people smarter than me. Most are scar tissue.

Taste evolves. What I believed five years ago about testing, abstraction, and "clean" code isn't what I believe now, and I expect the same gap five years from here. So this book is versioned, not carved. Each edition is a snapshot of what holds up today, and the revisions record what changed.

This book is also available as plain markdown at /taste.md and indexed in /llms.txt for machine readers.

## Chapter 1 — Foundation: Communication & Growth

### 1:1 Communicate to Collaborate
...
### 1:2 Learn the Rules to Break Them Wisely
...
Methodologies exist to solve problems, not to be followed blindly. The mark of maturity is knowing which rules serve your current context and which ones are getting in the way. Same idea as [adapting to context](#55-adapt-your-approach-to-the-context): both come down to understanding the "why" behind the "what."

### 1:3 Study Your Environment, Not Just Your Code
...
### 1:4 Embrace Discomfort to Grow
...
### 1:5 Own Problems, Propose Solutions
...
This mirrors the [Hollywood Principle](#42-hollywood-principle-dont-call-us-well-call-you) in modular design: don't wait for someone else to fix your problems, but also don't reach into other people's domains uninvited. Own your boundary, propose across boundaries, collaborate at the interfaces.
...
## Chapter 2 — Software Craft: Quality & Simplicity

### 2:1 Build Quality In
...
### 2:2 Understand Needs, Not Just Requirements
...
### 2:4 Code Never Dies, You Have to Kill It
...
### 2:5 The Code That Changes Together Stays Together
...
### 2:6 Don't Guess Where the Bottlenecks Are
...
### 2:7 Measure Before You Optimize

Measure. Don't tune for speed until you've measured, and even then don't unless one part of the code overwhelms the rest.

### 2:8 Keep It Simple When N Is Small
...
### 2:10 Let Data Drive Your Design
...
### 3:1 Treat Every External Interface as a Trust Boundary
...
### 3:2 Question Schemas, Don't Worship Them
...
This echoes [understanding needs over requirements](#22-understand-needs-not-just-requirements). Schemas document intent imperfectly, just like requirements. Trust, but verify.

### 3:3 Validate to Defend, Not Just to Parse
...
### 3:4 Invest in Validation for Better DX
...
### 4:1 Separation of Policy from Mechanism
...
### 4:2 Hollywood Principle: "Don't Call Us, We'll Call You"
...
This is the foundation of [dependency injection](https://en.wikipedia.org/wiki/Dependency_injection) and inversion of control. Your component shouldn't reach across boundaries to solve problems outside its domain. See also [owning problems](#15-own-problems-propose-solutions).
...
### 4:5 Interface Segregation: Pass Only What's Needed
...
Ask objects to provide what you need directly, or extract values at the boundary. Same idea as [interface segregation](#45-interface-segregation-pass-only-whats-needed): both push you toward minimal, explicit dependencies.

## Chapter 5 — Leadership

### 5:1 Lead by Crafting, Not Just Directing
...
Senior engineers aren't more comfortable with complexity because they're smarter. They're more systematic. They know when to [simplify](#23-simplify-relentlessly) and when to accept necessary complexity. The art is knowing the difference.

### 5:3 Shape Culture Through Empathy
...
Your ability to influence depends on your ability to understand others' perspectives. Empathy is the foundation of [communication](#11-communicate-to-collaborate).

### 5:4 Transform Complexity into Clarity
...
### 5:5 Adapt Your Approach to the Context
...
This echoes [learning the rules to break them](#12-learn-the-rules-to-break-them-wisely). Both require reading context instead of applying rigid patterns. One-size-fits-all solutions are a sign of inexperience.

### 5:6 Honor the Systems You Inherit
...
This connects to [killing dead code](#24-code-never-dies-you-have-to-kill-it). Respect existing code enough to understand it, but be willing to delete it when its time has passed.

### 5:7 Learn Continuously, Teach Broadly
...
### 5:8 Create Lasting Impact Beyond Code
...
## Chapter 6 — Concrete Taste

*This chapter is a working draft — I am still editing it.*

### 6:1 Prefer strict TypeScript, ban `any` and enums

I run TypeScript in strict mode on every project. `any` is a type-system lie — it silences the compiler and pushes errors into runtime. Enums are a leaky abstraction that compile to objects you can't easily introspect; union types and `as const` objects do the same job with less magic.

If you can't type it without `any`, that's a signal the design needs work, not a reason to opt out of safety.

### 6:2 Named exports only

I never use default exports. Named exports make refactoring reliable — the editor finds every reference without guesswork, and the module's public surface is explicit. Default exports encourage inconsistent naming across import sites and hide what a module actually exposes.

If a module exports one thing, name it and export it by name.

### 6:3 Named function components, never arrow assignments

I write React components as named function declarations. Arrow-assigned components lose their name in stack traces and devtools, which makes debugging harder. Named functions are also easier to move, split, and test in isolation.

`const Foo = () => {}` looks like a component but behaves like an anonymous function. `function Foo() {}` is unambiguous.

### 6:4 Type aliases over interfaces

I use `type` for everything. Interfaces were designed for declaration merging — a feature I almost never need and occasionally find surprising. Type aliases are more consistent, support unions and intersections naturally, and compose the same way across the entire codebase.

When the whole team uses one form, the codebase is easier to read.

### 6:5 Parse at the boundary, don't validate inside

I validate at system boundaries — network responses, form inputs, environment variables — using Valibot schemas. Once data passes that boundary, the rest of the code treats it as fully typed and asks no further questions.

Scattering `if (x !== undefined)` guards through business logic means the parsing failed: something unknown entered the system and you're managing it with runtime checks instead of types.

### 6:6 Integration tests over unit tests

I test user behaviour, not implementation details. An integration test that exercises a full slice of the system — from input to output — gives me confidence that the feature works. A unit test that checks a private function gives me confidence that the function works, which is rarely the thing I care about.

Unit tests make refactoring harder; integration tests make it safer.

### 6:7 Self-documenting code first, TSDoc where it counts

I name things so the code reads as a statement of intent. A comment that explains what the code does is a signal that the code should be clearer. A comment that explains why a constraint exists — a hidden invariant, an upstream quirk — is worth writing.

TSDoc belongs on public APIs where the caller can't see the implementation. Everywhere else, the code is the documentation.

### 6:8 Declarative over imperative

I reach for imperative code only when the abstraction doesn't exist yet. Declarative code describes what should happen; the reader doesn't have to simulate execution to understand it. When I find myself writing a loop to build a list, I ask whether `map`, `filter`, or `reduce` says the same thing more directly.

The right abstraction makes intent visible and detail invisible.

### 6:9 Composition over inheritance

I build behaviour by combining small, focused pieces — not by reaching up an inheritance chain. Inheritance couples you to an ancestor you don't fully control; composition lets you swap parts independently. Most of the time, a hook, a higher-order component, or a plain function covers what inheritance would have done with less ceremony.

I treat extending a class as a last resort, not a default pattern.

### 6:10 Duplication is cheaper than the wrong abstraction

I don't reach for an abstraction until I see the same shape three times and understand exactly what varies. A premature abstraction couples unrelated callers through a shared interface neither of them chose. When that interface needs to change for one caller, it breaks the others.

Copy the code. Wait. When the pattern is clear, the right abstraction is obvious.
